...
function activate(context) {
	...
	let clickBar = vscode.commands.registerCommand(
		'extension.clickStatusBar',
		() => {
			vscode.window
				.showQuickPick(Object.keys(env['books']))
				.then((value) => {
					console.log(value)
	...
	vscode.window.onDidChangeActiveTextEditor((editor) => {
		...
		if (editor) {
			...
			if (
				fspath &&
				fspath.toLowerCase() ===
					path.join(__dirname, 'novel.js').toLowerCase()
			) {
				processBar.show()
				prevBar.show()
				nextBar.show()
...
function initNovelInfo() {
	var content = fs.readFileSync(
		path.join(__dirname, 'books', env['currentBook'])
	)
	novelLines = content.toString().split('\n')
	// const filterr = filter.map(f => new RegExp(`${f}`))
	// filterr.forEach(f => {
	//     remove(novelLines, line => {
	//         return f.test(line)
	//     })
	// })
	totalPage = Math.ceil(novelLines.length / settings.pageSize)
}
// 初始化状态栏
function initStatusBar() {
	const message = `${env['currentBook']}   ${
...
function getTrueTitle(curr, i = 0) {
	let currLines = novelLines
		.slice(
			(curr - 1 - i) * settings.pageSize,
			(curr - i) * settings.pageSize
		)
		.find((line) => settings.titleReg.test(line))
	if (currLines) {
		return currLines
	} else {
		i++
		return getTrueTitle(curr, i)
	}
	return backLines ? backLines : getTrueTitle(curr, i)
}

// 获取当前页数据
function getPage() {
	const curr = env['books'][env['currentBook']]
	const lines = novelLines.slice(
		(curr - 1) * settings.pageSize,
		curr * settings.pageSize
	)
	console.log(lines)
	var template = tpl.format(...lines)
	fs.writeFileSync(novelPath, template)
	if (processBar) {
		processBar.text = `${env['currentBook']} - ${getTrueTitle(
			curr
		)} - ${curr} | ${totalPage}`
	}
	scrollToTop()
...
function nextPage() {
	...
	if (
		fspath &&
		fspath.toLowerCase() !== path.join(__dirname, 'novel.js').toLowerCase()
	) {
		return
	}
